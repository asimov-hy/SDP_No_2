...
class TestPlayerLevelUpCore:
    ...
    def test_multiple_consecutive_level_ups_with_overflow(
        self, mock_player_with_levelup
    ):
        """
        ...
        event = EnemyDiedEvent(
            position=(0, 0), enemy_type_tag="boss_enemy", exp=massive_exp
        )
        player._on_enemy_died(event)

        # Should have multiple level-ups
        assert player.level > initial_level + 1, (
            f"Expected multiple level-ups, only got to level {player.level}"
        )
        assert player.exp >= 0, f"EXP overflow should be non-negative, got {player.exp}"

        # Should update session stats
        stats = (
            mock_player_with_levelup._mock_session_stats
            if hasattr(mock_player_with_levelup, "_mock_session_stats")
            else None
        )
        if stats:
            assert stats.total_exp_gained >= massive_exp

    def test_experience_calculation_accuracy_across_enemy_types(
        self, mock_player_with_levelup
    ):
        """
        ...
        player = mock_player_with_levelup
        initial_level = player.level
        initial_total_gained = (
            player._mock_session_stats.total_exp_gained
            if hasattr(player, "_mock_session_stats")
            else 0
        )
        ...
        for enemy_type, exp_value in enemy_scenarios:
            event = EnemyDiedEvent(
                position=(0, 0), enemy_type_tag=enemy_type, exp=exp_value
            )
            player._on_enemy_died(event)
            ...
            assert player.exp >= 0, (
                f"EXP should never be negative after killing {enemy_type}, got {player.exp}"
            )

        # Verify that massive EXP gain from diverse enemies triggers level-ups
        assert player.level > initial_level, (
            f"Should have leveled up from killing diverse enemies, still level {player.level}"
        )
        ...
        if hasattr(player, "_mock_session_stats") and player._mock_session_stats:
            assert (
                player._mock_session_stats.total_exp_gained
                >= initial_total_gained + total_expected_exp
