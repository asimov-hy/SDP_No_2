...
class EnemySpeed(BaseEnemy):
    __registry_category__ = EntityCategory.ENEMY
    __registry_name__ = "speed"
    ...
    def __init__(self, x, y, direction=(0, 1), speed=None, health=None,
        ...
        defaults = EntityRegistry.get_data("enemy", "speed")

        # Apply overrides or use defaults
        speed = speed if speed is not None else defaults.get("speed", 280)
        health = health if health is not None else defaults.get("hp", 1)
        scale = scale if scale is not None else defaults.get("scale", 1.0)
        score = score if score is not None else defaults.get("score", 30)
        ...
        DebugLogger.init(
            f"Spawned EnemySpeed at ({x}, {y}) | Speed={speed}",
    ...
    def reset(self, x, y, direction=(0, 1), speed=None, health=None, scale=None, score=None, **kwargs):
        ...
        DebugLogger.system(
            f"reset EnemySpeed at ({x}, {y}) | Speed={speed}",
            category="entity_spawn"
        )